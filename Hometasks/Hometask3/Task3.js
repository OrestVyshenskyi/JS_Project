...
// ШАБЛОН
// <div class="product-card">//     <h3 class="product-title">TITLE. Price - PRICE</h3>
...
for (const user of users) {
    if (user.status === true){
        document.write(`<div>`);
        document.write(`<h4>${user.name} STATUS - ${user.status}</h4>`)
        document.write(`</div>`);
    }
}
...
for (const user of users){
    if (user.age > 30){
        document.write(`<div>`);
        document.write(`<h4>${user.name} AGE - ${user.age}</h4>`)
        document.write(`</div>`);
    }
}
